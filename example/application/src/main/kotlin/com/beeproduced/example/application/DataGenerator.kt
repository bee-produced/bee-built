 ...
 */
@Component
class DataGenerator(private val eventManager: EventManager) {
  ...
  private fun setupCompanies() {
    companies =
      persons.chunked(2).drop(1).map { p ->
        val create =
          CreateCompanyInput(
            faker.company.name(),
            CreateAddressInput(
  ...
  private fun <V> AppResult<V>.getOrFail(): V {
    return getOrThrow { e ->
      IllegalStateException("Data generation failed: ${e.stackTraceToString()}")
    }
  }
}
