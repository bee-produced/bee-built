 ...
 */
@Service
class FilmService(private val eventManager: EventManager, private val repository: FilmRepository) {
  private val logger = logFor<FilmRepository>()

  @TransactionalResult("mediaTransactionManager", exceptionDescription = "Could not create film")
  fun create(create: CreateFilmInput, selection: DataSelection): AppResult<Film> {
    logger.debug("create({}, {})", create, selection)
    return organisationIdsExist(create.studios, create.directors + create.cast).map {
      repository.persist(
        Film(
          UUID.randomUUID(),
          create.title,
  ...
  fun organisationIdsExist(
    ...
    val companyResult =
      if (companyIds.isNullOrEmpty()) Ok(Unit) else eventManager.send(CompaniesExist(companyIds))
    return companyResult.andThen {
      if (personIds.isNullOrEmpty()) Ok(Unit) else eventManager.send(PersonsExist(personIds))
    }
  }

  @TransactionalResult("mediaTransactionManager", exceptionDescription = "Could not update film")
  fun update(update: UpdateFilmInput, selection: DataSelection): AppResult<Film> {
    logger.debug("update({}, {})", update, selection)
    return repository
      .selectById(update.id)
      .toResultOr { BadRequestError("No film with id ${update.id} found") }
      .andThenOnSuccess {
        val personIds =
          mutableListOf<PersonId>()
