...
class BeeFetchedCodegen(
  ...
  private fun FileSpec.Builder.buildAutoFetcher(): FileSpec.Builder {
    addType(
      TypeSpec.classBuilder(fileName).addAnnotation(DgsComponent::class).buildAutoFetcher().build()
    )
    return this
  }

  private fun TypeSpec.Builder.buildAutoFetcher(): TypeSpec.Builder {
    for ((name, dto) in dtos) processDto(name, dto)
    return this
  }
  ...
  private fun TypeSpec.Builder.processDto(name: String, dto: DgsDto): TypeSpec.Builder {
    ...
    for (property in dto.properties) {
      if (property.nonCollectionType != definition.dtoType) {
        logger.info(
          "processDto|skipping property: ${property.nonCollectionType} != ${definition.dtoType}"
        )
        continue
      }
      ...
      val (internalName, idProperty, hasProperty) =
        if (internalDto == null) {
      ...
      if (idProperty == null) {
        logger.warnError(
          "processDto|no id property found for ${internalDto ?: name} - ${property.name}"
        )
        continue
      }
      if (idProperty.isCollection != property.isCollection) {
        logger.warnError(
          "processDto|mismatch between id property & property for ${internalDto ?: name} detected"
        )
        ...
        logger.warnError(
          "processDto|${idProperty.name} [$idPropertyKind] vs ${property.name} [$propertyKind]"
        )
        continue
      }
      ...
      addFunction(buildNestedFetcher(name, internalName, property, hasProperty, idProperty))
    }
  ...
  private fun buildNestedFetcher(
    ...
    val returnType = CompletableFuture::class.asClassName().parameterizedBy(propertyType)
    ...
    val dataLoaderType =
      DataLoader::class.asClassName().parameterizedBy(dataLoaderKeyType, dataLoaderDtoType)
    ...
    return FunSpec.builder(funcName)
      ...
      .apply {
        if (safeMode && hasProperty && property.isCollection)
          addNStatement(
            "if (!data.$_PROPERTY.isNullOrEmpty()) return $COMPLETABLE_FUTURE.completedFuture(data.$_PROPERTY)"
          )
      ...
      .apply {
        if (property.isCollection) {
          ...
          if (idProperty.isNullable && hasProperty)
            addNStatement(
              "if (ids.isNullOrEmpty()) return $COMPLETABLE_FUTURE.completedFuture(data.$_PROPERTY)"
            )
          ...
          if (idProperty.isNullable && hasProperty)
            addNStatement(
              "if (id == null) return $COMPLETABLE_FUTURE.completedFuture(data.$_PROPERTY)"
            )
          else if (idProperty.isNullable && !dataLoaderKeyType.isNullable) { // && !hasProperty
  ...
  private fun ignoreDto(name: String): Boolean {
    ...
    val ignore = ignoreDefinitions != null && ignoreDefinitions.any { it.property == null }
  ...
  private fun ignoreDtoProperty(name: String, property: String): Boolean {
    ...
    val ignoreDefinitions = typedIgnore[name]
    val ignore = ignoreDefinitions != null && ignoreDefinitions.any { it.property == property }
  ...
  private fun idNames(name: String, property: PropertyDetails): Set<String> {
    ...
    if (mapping != null) return setOf(mapping.idProperty)
    ...
    val idNames =
      when {
        property.isCollection && property.name.endsWith("s") ->
          setOf("${property.name}Ids", "${property.name.dropLast(1)}Ids")
