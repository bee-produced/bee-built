...
/**
 * EventManager = Mediator + Scheduler Allows easy communication between services with loose
 * coupling.
 *
 * Inspired by https://www.codeproject.com/Articles/35277/MVVM-Mediator-Pattern and
 * https://www.baeldung.com/java-delay-code-execution#service. Additional info found in
 * https://discuss.kotlinlang.org/t/how-to-bridge-java-runnable-with-kotlin-closures/996/3.
 *
...
open class SimpleEventManager : EventManager {
  ...
  override fun unregister(handler: RequestHandler<*, *>) {
    if (!requestHandlers.containsKey(handler.requestClass)) {
      logger.warn(
        "No request handler for request [${handler.requestClass.typeName}] " + "to be unregistered"
      )
    } else {
  ...
  override fun <T> send(request: Request<T>): Result<T, AppError> {
    return if (!requestHandlers.containsKey(request.javaClass)) {
      ...
      val handler = requestHandlers[request.javaClass]!! as RequestHandler<Request<T>, T>
  ...
  override fun register(handler: NotificationHandler<*>) {
    ...
    // [${notificationHandlers[handler.notificationClass]?.javaClass?.typeName}] " +
  ...
  override fun notify(notification: Notification) {
    if (!notificationHandlers.containsKey(notification.javaClass)) {
      throw NotificationHandlerNotFound(
        "No notification handlers for notification [${notification.javaClass}] found"
      )
      ...
      logger.info(
        "Invoking ${handlers.count()} handler(s) for notification [${notification.javaClass.typeName}]"
      )
      for (handlerToCast in handlers) {
        @Suppress("UNCHECKED_CAST") val handler = handlerToCast as NotificationHandler<Notification>
        logger.debug("Invoking handler [${handler.javaClass.typeName}]")
  ...
  //     if (!unregister)
  //         logger.warn("Could not unregister callback [${callback.javaClass.typeName}] for event
  // $event")
