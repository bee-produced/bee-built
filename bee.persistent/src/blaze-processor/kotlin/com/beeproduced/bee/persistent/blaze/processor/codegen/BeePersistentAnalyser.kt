...
data class EntityViewInfo(
  ...
  override val qualifiedName
    get() = entity.qualifiedName

  override val declaration: KSClassDeclaration
    get() = entity.declaration

  override val uniqueName: String
    get() = entity.uniqueName
}

data class EmbeddedViewInfo(override val name: String, val embedded: EmbeddedInfo) : BaseViewInfo {
  override val qualifiedName
    get() = embedded.qualifiedName

  override val declaration: KSClassDeclaration
    get() = embedded.declaration

  override val uniqueName: String
    get() = embedded.uniqueName
}
...
data class ViewInfo(
  ...
  val coreEntityViewsByQualifiedName: Map<String, EntityViewInfo> =
    entityViews.values.filter { it.name.endsWith("Core") }.associateBy { it.qualifiedName }

  val subclassEntityViewsBySuperClass =
    entityViews.values
      .filter { it.superClassName != null }
      .groupBy { requireNotNull(it.superClassName) }

  val entityViewCoreNames
    get() = entityViews.keys.filter { it.endsWith("Core") }
}

class BeePersistentAnalyser(private val logger: KSPLogger, config: BeePersistentBlazeConfig) {
  ...
  fun processEntities(entities: List<EntityInfo>): ViewInfo {
    ...
    return entities
      ...
      .reduce { acc, next ->
        ViewInfo(acc.entityViews + next.entityViews, acc.embeddedViews + next.embeddedViews)
      }
  }
  ...
  private fun processEntity(root: EntityInfo): ViewInfo {
    ...
    fun dfs(
      ...
      for (subClass in start.subClasses) {
        ...
        val viewName = if (isCore) viewName(subEntity, root) else viewName(subEntity, root, c)
        count[subEntity.qualifiedName!!] = c + 1
        val subInfo =
          EntityViewInfo(name = viewName, entity = subEntity, superClassName = startViewName)
    ...
    if (root.superClass != null) return ViewInfo(emptyMap(), emptyMap())
        ...
        for (info in extendRound) {
          if (i < depth) {
            for (relation in info.entityRelations) {
              ...
              info.relations[relation.simpleName] = viewName
              extend.add(
                EntityViewInfo(name = viewName, entity = relationEntity, isExtended = true)
              )
            }
          }
