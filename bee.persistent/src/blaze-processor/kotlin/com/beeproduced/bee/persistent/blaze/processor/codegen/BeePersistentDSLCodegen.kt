...
class BeePersistentDSLCodegen(
  ...
  init {
    ...
    poetMap.addMapping(
      VALUE_PATH,
      ClassName("com.beeproduced.bee.persistent.blaze.dsl.path", "ValuePath"),
    )
    poetMap.addMapping(PATH, ClassName("com.beeproduced.bee.persistent.blaze.dsl.path", "Path"))
    poetMap.addMapping(
      VALUE_PATH,
    ...
    poetMap.addMapping(
      EXP,
      ClassName("com.beeproduced.bee.persistent.blaze.dsl.expression", "Expression"),
    )
    poetMap.addMapping(
      VALUE_EXP,
      ClassName("com.beeproduced.bee.persistent.blaze.dsl.expression", "ValueExpression"),
    )
    poetMap.addMapping(
      SORTABLE_EXP,
      ClassName("com.beeproduced.bee.persistent.blaze.dsl.path", "SortableExpression"),
    )
    poetMap.addMapping(
      SORTABLE_VALUE_EXP,
      ClassName("com.beeproduced.bee.persistent.blaze.dsl.path", "SortableValueExpression"),
    )
    poetMap.addMapping(COMPONENT, ClassName("org.springframework.stereotype", "Component"))
    poetMap.addMapping(
      EVENT_LISTENER,
      ClassName("org.springframework.context.event", "EventListener"),
    )
    poetMap.addMapping(
    ...
    poetMap.addMapping(
      INLINE_VALUE_UNWRAPPERS,
      ClassName("com.beeproduced.bee.persistent.blaze.meta.dsl", "InlineValueUnwrappers"),
    )
    poetMap.addMapping(
      APPLICATION_LISTENER,
      ClassName("org.springframework.context", "ApplicationListener"),
    )
    poetMap.addMapping(
      APPLICATION_STARTING_EVENT,
      ClassName("org.springframework.boot.context.event", "ApplicationStartingEvent"),
    )
  }
  ...
  private fun FileSpec.Builder.buildSelectionDSL(): FileSpec.Builder = apply {
    ...
    val selectionDSL = TypeSpec.objectBuilder(selectionDSLName)
  ...
  private fun TypeSpec.Builder.traverseDSL(
    ...
    val viewDSLFields =
      PropertySpec.builder(
        "fields",
        MUTABLE_SET.parameterizedBy(poetMap.classMapping(TYPED_FIELD_NODE)),
      )
  ...
  private fun TypeSpec.Builder.traverseDSL(
    ...
    if (someProp != null) {
      ...
      viewDSLFields.initializer(
        CodeBlock.builder()
          .addNamedStmt(
            "mutableSetOf(${TYPED_FIELD_NODE}(\"$simpleName\", \"$embeddedSimpleName\"))"
          )
          .build()
      )
    } else {
      viewDSLFields.initializer(CodeBlock.builder().addNamedStmt("mutableSetOf()").build())
    }
  ...
  private fun TypeSpec.Builder.traverseWhereDSL(
    ...
    val (allRelations, allColumns) = viewColumnsWithSubclasses(entityView, views)
    ...
    for ((simpleName, subRelation) in allRelations) {
      ...
      val subViewMapping =
        if (subRelation.subView == null) null
      ...
      val relationProperty =
        PropertySpec.builder(simpleName, ClassName("", whereDSLName))
  ...
  private fun TypeSpec.Builder.addPath(
    ...
    if (inner != null) {
      ...
      val interfaceType =
        if (sort) poetMap.classMapping(SORTABLE_VALUE_EXP).parameterizedBy(columnType, innerType)
        else poetMap.classMapping(VALUE_EXP).parameterizedBy(columnType, innerType)
      val pathType = poetMap.classMapping(VALUE_PATH)

      val pathProperty =
        PropertySpec.builder(propertyName, interfaceType)
          .initializer(
            CodeBlock.Builder()
    ...
    val columnType = column.type.toTypeName().copy(nullable = false)
    val interfaceType =
      if (sort) poetMap.classMapping(SORTABLE_EXP).parameterizedBy(columnType)
      else poetMap.classMapping(EXP).parameterizedBy(columnType)
    val pathType = poetMap.classMapping(PATH)
    val pathProperty =
      PropertySpec.builder(propertyName, interfaceType)
        .initializer(CodeBlock.Builder().addStatement("%T(%S)", pathType, newPath).build())
    addProperty(pathProperty.build())
    ...
    for (embeddedColumn in embedded.jpaProperties) {
      val columnProp =
        ColumnProperty(
          embeddedColumn.declaration,
          embeddedColumn.type,
          embeddedColumn.annotations,
          null,
          null,
        )
  ...
  private fun buildPath(path: String, simpleName: String, subViewMapping: SubViewMapping?): String {
    return when {
      ...
      path.isEmpty() && subViewMapping != null -> {
        // TREAT alias like CriteriaBuilder<T> create function from blaze: camel cased result of
        // what Class.getSimpleName()
      ...
      subViewMapping == null -> {
        ...
        "TREAT($path as ${subViewMapping.subEntitySimpleName}).$simpleName"
      }
    }
  }
  ...
  private fun buildRegistration() {
    ...
    FileSpec.builder(packageName, unwrapperClassName)
      .buildRegistration()
      .build()
      .writeTo(codeGenerator, dependencies)
  }
  ...
  private fun FileSpec.Builder.buildRegistration() = apply {
    ...
    val listenerInterface = poetMap.classMapping(APPLICATION_LISTENER).parameterizedBy(event)

    val registrationName = buildUniqueClassName(packageName, "UnwrapperRegistration")
    val registration = TypeSpec.classBuilder(registrationName).addSuperinterface(listenerInterface)

    val register =
      FunSpec.builder("onApplicationEvent")
    ...
    register.apply {
      ...
      for (unwrapProperty in inlineValues.values) {
        ...
        addNamedStmt(
          "$INLINE_VALUE_UNWRAPPERS.registerUnwrapper($INLINE_VALUE_CLAZZ::class.java, $INNER_CLAZZ::class.java)"
        )
      }
    }
