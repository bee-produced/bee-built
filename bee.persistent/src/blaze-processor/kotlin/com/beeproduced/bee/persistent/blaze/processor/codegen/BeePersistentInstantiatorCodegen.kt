...
class BeePersistentInstantiatorCodegen(
  ...
  private fun FileSpec.Builder.addNamedStmt(format: String) = addNStatementBuilder(format, poetMap)
  ...
  init {
    ...
    poetMap.addMapping(
      BLAZE_INSTANTIATORS,
      ClassName("com.beeproduced.bee.persistent.blaze.meta.proxy", "BlazeInstantiators"),
    )
    poetMap.addMapping(
      TCI,
      ClassName("com.beeproduced.bee.persistent.blaze.meta.proxy", "TupleConstructorInstantiator"),
    )
    poetMap.addMapping(
      ACI,
      ClassName(
        "com.beeproduced.bee.persistent.blaze.meta.proxy",
        "AssignmentConstructorInstantiator",
      ),
    )
  ...
  private fun FileSpec.Builder.buildAssignmentCreator(
    view: EntityViewInfo,
    assignmentCreators: MutableMap<String, String>,
  ) = apply {
    ...
    val property = PropertySpec.builder(propertyName, poetMap.classMapping(ACI))
      ...
      CodeBlock.builder()
        .apply {
          ...
          for (field in missingFields) {
            addStatement(
              "    val ${field.simpleName} = null as %T",
              field.declaration.type.toTypeName(),
            )
          }
  ...
  private fun FileSpec.Builder.buildTupleCreator(
    ...
    val property = PropertySpec.builder(propertyName, poetMap.classMapping(TCI))
    ...
    val initializer =
      CodeBlock.builder()
        .apply {
          ...
          // Needed because BlazeTupleInstantiatorPatch.newInstance tries to create embedded columns
          // on
  ...
  private fun CodeBlock.Builder.addField(index: Int, field: Property, hasSort: Boolean = false) {
    ...
    if (field.isValueClass) {
      ...
      if (!field.type.isMarkedNullable) {
        addStatement(
          "    val ${field.simpleName} = %T(tuple[$tupleAccess] as %T)",
          field.type.toTypeName(),
          innerValue.type.toTypeName(),
        )
      } else {
        addStatement(
          "    val ${field.simpleName} = (tuple[$tupleAccess] as %T).let { %T(it) }",
          innerValue.type.toTypeName(),
          field.type.toTypeName(),
        )
      }
  ...
  private fun FileSpec.Builder.buildInfo(entityInfo: BaseInfo) = apply {
    ...
    for (field in construction.reflectionProps) {
      val setterProp =
        PropertySpec.builder(infoField(field), poetMap.classMapping(FIELD))
          .initializer(
  ...
  private fun FileSpec.Builder.buildRegistration(
    ...
    val event =
      poetMap.classMapping(BeePersistentDSLCodegen.PoetConstants.APPLICATION_STARTING_EVENT)
    val listenerInterface =
      poetMap
        .classMapping(BeePersistentDSLCodegen.PoetConstants.APPLICATION_LISTENER)
  ...
  private fun BaseInfo.construction(): EntityConstruction {
    ...
    val jpaProperties =
      jpaProperties.associateByTo(HashMap(jpaProperties.count())) { it.simpleName }
  ...
  private fun infoName(uniqueName: String): String = "${uniqueName}InstatiatorInfo"

  private fun infoField(prop: EntityProperty): String = "${prop.simpleName}Field"
  ...
  private fun BaseViewInfo.entityFields(): List<Property> {
    return when (this) {
      is EntityViewInfo -> {
        val id = entity.id
        val props =
          (entity.columns + entity.lazyColumns + entity.relations).sortedBy { it.simpleName }
        listOf(id) + props
      }
      is EmbeddedViewInfo -> {
        (embedded.columns + embedded.lazyColumns).sortedBy { it.simpleName }
