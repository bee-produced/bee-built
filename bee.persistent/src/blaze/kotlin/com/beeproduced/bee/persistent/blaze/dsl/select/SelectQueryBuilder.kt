...
class SelectQueryBuilder<T : Any> : SelectQuery<T> {
  ...
  override fun where(predicate: Predicate): SelectOrderBy<T> = apply { where = predicate }
  ...
  override fun orderBy(vararg sorts: Sort): Selection<T> = apply { orderBy = sorts.toList() }

  fun applyBuilder(builder: CriteriaBuilder<T>): CriteriaBuilder<T> {
    var b: CriteriaBuilder<T> = builder
    where?.run { b = applyBuilder(b) }
    orderBy?.let { sorts -> for (sort in sorts) sort.run { b = applyBuilder(b) } }
    return b
  }
}
