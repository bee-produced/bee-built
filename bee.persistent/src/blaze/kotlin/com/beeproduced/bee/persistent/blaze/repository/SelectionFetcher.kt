...
private fun EntityViewSetting<*, *>.fetchSelection(
  path: String,
  info: SelectionInfo,
  selection: BeeSelection,
) {
  if (info.id != null) fetch<Any>("$path${info.id}")
  for (column in info.columns) fetch<Any>("$path$column")
  for (lazyColumn in info.lazyColumns) if (selection.contains(lazyColumn))
...
private fun EntityViewSetting<*, *>.fetchSubselection(
  path: String,
  subInfo: Map<String, SelectionInfo>,
  selection: BeeSelection,
) {
  for ((property, info) in subInfo) {
    val subSelection = selection.subSelect(property) ?: DefaultBeeSelection(emptySet())
    fetchSelection("$path$property.", info, subSelection)
  }
}

private fun EntityViewSetting<*, *>.fetchLazySubselection(
  path: String,
  subInfo: Map<String, SelectionInfo>,
  selection: BeeSelection,
) {
  for ((property, info) in subInfo) {
    val subSelection = selection.subSelect(property) ?: continue
    fetchSelection("$path$property.", info, subSelection)
  }
}
