...
object QueryBuilder {
   ...
   */
  fun whereClauseFromEntity(entity: Any, entityType: Class<*>, ids: Ids): PredicateSpec? {
  ...
  /**
   * Builds a `WHERE` clause for the table of `entityType` with the values from the given `id`. The
   * `id` can be primitive e.g., `Long` or a class that is used as an [javax.persistence.IdClass].
   */
  ...
  /**
   * Builds a `WHERE` clause for the table of `entityType` with the values from the given `id`. The
   * `id` must be a primitive e.g., `Long`
   */
  ...
  /**
   * Builds a `WHERE` clause for the table of `entityType` with the values from the given composite
   * `id`. The `id` must be a class that is used as an [javax.persistence.IdClass].
   */
  private fun whereClauseFromCompositeKey(
    ...
    if (idInfo is List<*>)
      return whereClauseFromCompositeKeyList(entityType, compositeKeyMapping, idInfo)
    var whereClause: PredicateSpec? = null
  ...
  private fun whereClauseFromCompositeKeys(
    ...
    for (id in ids) {
      val clause = whereClauseFromCompositeKey(entityType, compositeKeyMapping, id) ?: continue
      whereClauses.add(clause)
    }
    return OrSpec(whereClauses)
  }

  fun updateSetParams(entity: Any, entityType: Class<*>, fields: Fields): Map<ColumnSpec<*>, Any?> {
    // Equal to following criteria DSL that can be used in update query:
    // mapOf(column(EntityType::field1) to entity.field1, column(EntityType::field2) to
    // entity.field2..
  ...
  fun selectDistinctIdsFromEntity(
    ...
    return if (ids.values.count() == 1) {
      ...
      val multiSelectQuery: CriteriaQueryDsl<*>.() -> Unit = { this.selectDistinct(columns) }
      multiSelectQuery
    }
  }
