...
abstract class BaseDataRepository<T : DataEntity<T>, ID : Any>(
  ...
  init {
    ...
    @Suppress("UNCHECKED_CAST")
    selectDistinctIdColumns =
      QueryBuilder.selectDistinctIdsFromEntity(this.type, ids) as CriteriaQueryDsl<*>.() -> Unit
    @Suppress("UNCHECKED_CAST")
    selectDistinctIdAndOrderColumns =
      QueryBuilder.selectDistinctIdAndOrderColumnsFromEntity(this.type, ids)
  ...
  open fun persist(entity: T): T {
    ...
    // -----------------------------------------------
    // For assigned identifiers, a merge will always require an SQL SELECT since Hibernate cannot
    // know
    // if there is already a persisted entity having the same identifier. For other identifier
    // generators,
  ...
  /**
   * Inserts multiple new entities into db. Use [update] to update an already persisted entity, not
   * [persist].
   */
  ...
  /**
   * Inserts new entity into db and queries it again with specified relations loaded. Use [update]
   * to update an already persisted entity, not [persist].
   */
   ...
   */
  open fun persistAllAndSelect(
    entities: Collection<T>,
    selection: DataSelection = EmptySelection(),
  ): List<T> {
    val entityIds = entities.map { entity -> persist(entity) }.map { entity -> identifier(entity) }
   ...
   * [javax.persistence.OneToOne] and so on will NOT be updated.
   */
  ...
  /**
   * Updates all non-relation dependent fields of an entity in db and queries it again with
   * specified relations loaded. Fields marked with [javax.persistence.OneToOne] and so on will NOT
   * be updated.
   */
  @Suppress("NAME_SHADOWING")
  ...
  /**
   * Queries entities with specified relations from db. If [ids] do not exist, empty list will be
   * returned. If only some [ids] are missing, a partial result will be returned.
   */
  ...
  /**
   * Queries entities with kotlin-jdsl [dsl] and specified relations from db. The [dsl] should NOT
   * contain a “SELECT” nor “FROM” clause.
   */
  open fun select(
    selection: DataSelection = EmptySelection(),
    ...
    val query =
      if (testDsl.hasLimitClause && !testDsl.hasOrderByClause) {
        ...
        val resultIds: List<Any> =
          queryFactory
            .selectQuery(info.nonPrimitiveIdType) {
        ...
        val idsAndOrderBy: List<List<Any>> =
          queryFactory
            .selectQuery<List<Any>> {
        ...
        val resultIds = idsAndOrderBy.map { it.dropLast(orderByColumns.count()) }
  ...
  open fun count(distinct: Boolean, dsl: CriteriaQueryDsl<*>.() -> Unit = {}): Long {
    val count =
      queryFactory.selectQuery(Long::class.javaObjectType) {
  ...
  /**
   * Returns true if for EVERY given ID an entity exists in the db. Just with one ID not found,
  ...
  /** Deletes entity with specified [id] from db. If id does not exist, nothing will be deleted. */
  open fun deleteById(id: ID): DeletedRowsCount {
    val query =
      queryFactory.deleteQuery(kType) {
        where(QueryBuilder.whereClauseFromId(type, info.compositeKeyMapping, id, ids))
      }
    return query.executeUpdate()
  }
  ...
  /**
   * Deletes all entities with specified [ids] from db. If [ids] do not exist, nothing will be
   * deleted. If only some [ids] are existent, then only them will be deleted.
   */
  ...
  /** Returns the ID of an entity. Useful for composite keys. */
  @Suppress("UNCHECKED_CAST")
