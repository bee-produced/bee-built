...
import com.beeproduced.data.jpa.exceptions.EntityNotFound
...
object MetaModel {
    ...
    fun addEntity(
        ...
        if (entities.containsKey(entityType))
            return entities.getValue(entityType)
        ...
        for (field in entityType.declaredFields) {
            ...
            if (MembersInfo.isFieldId(field)) {
                ...
                if (!isNullable) {
                    logger.warn("Relation [$member] of [$entityType] is not marked nullable, this can lead to errors when relation is lazy loaded")
                }
