...
@Component
class PaginatedFooRepository(@Qualifier("orderEntityManager") em: EntityManager) :
...
@Component
class PaginatedBarRepository(@Qualifier("orderEntityManager") em: EntityManager) :
...
@Component
class PaginatedFoxtrotRepository(@Qualifier("orderEntityManager") em: EntityManager) :
  ...
  private val barPagination =
    Pagination(
      repository = this,
      orderBy =
        ColumnSpec(EntitySpec(PaginatedFoxtrot::class.java), PaginatedFoxtrot::createdOn.name),
      cursor = Cursor(decode = ::decodeCursor, encode = ::encodeCursor),
...
@Component
class FoxtrotInfoRepository(@Qualifier("orderEntityManager") em: EntityManager) :
