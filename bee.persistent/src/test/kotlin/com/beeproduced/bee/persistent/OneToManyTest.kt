...
class OneToManyTest(
  ...
  @Qualifier("orderEntityManager") val em: EntityManager,
) {
  private val transaction = TransactionTemplate(transactionManager)

  @Configuration
  class TestConfig(val em: EntityManager) {
    @Bean fun workRepository(): WorkRepository = WorkRepository(em)

    @Bean fun collectionRepository(): WorkCollectionRepository = WorkCollectionRepository(em)
  }
  ...
  fun `select with joined where clause`() {
    ...
    val collectionWithWork2 =
      collectionRepo
        .select(FullNonRecursiveSelection()) {
  ...
  fun `select by ids with relation loaded`() {
    ...
    val selection = SimpleSelection(setOf(SimpleSelection.FieldNode(WorkCollection::works.name)))
    val collections = collectionRepo.selectByIds(listOf(collection1Id, collection2Id), selection)
  ...
  fun `persist all and select with relation loaded`() {
    ...
    transaction.executeWithoutResult {
      ...
      val works =
        workRepo.persistAllAndSelect(
          listOf(Work(collection1Id, "Hey!"), Work(collection2Id, "Moin!")),
          selection,
        )
  ...
  fun `exists with multiple ids`() {
    ...
    assertTrue(collectionRepo.existsAll(listOf(collection1Id, collection2Id, collection3Id)))
    assertFalse(collectionRepo.existsAll(listOf(collection1Id, collection2Id, collection3Id, -1)))
  }
  ...
  fun `delete by ids with composite key`() {
    ...
    transaction.executeWithoutResult {
      val rows =
        workRepo.deleteByIds(setOf(WorkId(work1Id, collection1Id), WorkId(work2Id, collection2Id)))
      assertEquals(2, rows)
    }
