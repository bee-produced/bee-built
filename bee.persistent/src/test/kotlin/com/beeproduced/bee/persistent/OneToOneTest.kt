...
class OneToOneTest(
  ...
  class TestConfig(val em: EntityManager) {
    @Bean fun rootRepository(): RootRepository = RootRepository(em)

    @Bean fun collectionRepository(): BranchRepository = BranchRepository(em)
  }
  ...
  fun `select with relations not loaded`() {
    ...
    transaction.executeWithoutResult {
      ...
      rootRepo.update(root.copy(branchAKey = branchA.id, branchBKey = branchB.id))
      branchRepo.update(branchA.copy(branchAKey = branchAA.id, branchBKey = branchAB.id))
    }
  ...
  fun `select with some relations loaded`() {
    ...
    transaction.executeWithoutResult {
      val selection =
        SimpleSelection(
          setOf(FieldNode(Root::branchA.name, setOf(FieldNode(Branch::branchA.name))))
        )
  ...
  fun `select recursive relation`() {
    ...
    transaction.executeWithoutResult {
      ...
      branchRepo.update(branchA.copy(branchAKey = branchA.id, branchBKey = branchAB.id))
    }
