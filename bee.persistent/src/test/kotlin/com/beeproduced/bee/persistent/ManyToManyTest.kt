...
class ManyToManyTest(
  ...
  @Autowired val fooBarRepo: FooBarRelationRepository,
  ...
  class TestConfig(val em: EntityManager) {
    @Bean fun fooRepository() = FooRepository(em)

    @Bean fun barRepository() = BarRepository(em)

    @Bean fun fooBarRepository() = FooBarRelationRepository(em)
  }
  ...
  fun `add relation`() {
    ...
    transaction.executeWithoutResult { fooBarRepo.persist(FooBarRelation(foo1Id, barId)) }
    ...
    transaction.executeWithoutResult { fooBarRepo.persist(FooBarRelation(foo2Id, barId)) }
  ...
  fun `delete relation`() {
    ...
    transaction.executeWithoutResult {
      fooBarRepo.delete { where(column(FooBarRelation::bar).equal(barId)) }
    }
