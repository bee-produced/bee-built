...
@PublishedApi
internal inline fun <V, reified E : AppError> Result<V, AppError>.mapInternalErrorGeneric(
    constructor: (String, ResultError, Long, Long) -> E, description: String
): Result<V, AppError> {
...
inline infix fun <V> Result<V, AppError>.mapInternalError(transform: (InternalAppError) -> InternalAppError): Result<V, AppError> {
...
inline infix fun <V> Result<V, AppError>.mapBadRequestError(transform: (BadRequestError) -> BadRequestError): Result<V, AppError> {
    return mapInternalErrorGeneric(transform)
}
