...
import com.beeproduced.bee.functional.extensions.com.github.michaelbull.result.mapWithPair
import com.beeproduced.bee.functional.extensions.com.github.michaelbull.result.mapWithTriple
...
class MapTest {
  ...
  private fun bar(str: String, str2: String, str3: String) =
    str.count() + str2.count() + str3.count()
  ...
  fun `map to pair and call function`() {
    ...
    val actualCount =
      Ok(str).map { Pair(it, str2) }.mapWithPair(::bar).getOrThrow { IllegalArgumentException() }
  ...
  fun `map to pair and call member function`() {
    ...
    val actualCount =
      Ok(foo).map { Pair(it, str) }.mapWithPair(Foo::bar).getOrThrow { IllegalArgumentException() }
