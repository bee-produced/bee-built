  ...
  init {
    ...
    if (aspectAnnotation == null) {
      throw IllegalStateException(
        "DGS-Error-Handling: [${this::class.simpleName}] must be annotated with [org.aspectj.lang.annotation.Aspect]"
      )
    }
    val dgsDataAnnotation = ::aroundDgsData.findAnnotation<Around>()
    if (dgsDataAnnotation == null) {
      throw IllegalStateException(
        "DGS-Error-Handling: Method [aroundDgsData] of [${this::class.simpleName}] must be annotated with [org.aspectj.lang.annotation.Around]"
      )
    }
    if (!dgsDataAnnotation.value.contains(aroundAnnotationExpression)) {
      throw IllegalStateException(
        "DGS-Error-Handling: Method [aroundDgsData] of [${this::class.simpleName}] must contain [$aroundAnnotationExpression] in [Around] annotation"
      )
    }
  }
...
@Aspect class DefaultAroundDgsDataAspect : AroundDgsDataAspect()
