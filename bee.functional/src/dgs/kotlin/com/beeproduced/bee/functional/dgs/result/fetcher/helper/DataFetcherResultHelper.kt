  ...
  companion object {
    ...
    fun <V> InternalErr(message: String, vararg formatArgs: Any): DataFetcherResult<V> {
      return DataFetcherResult.newResult<V>()
        .error(TypedGraphQLError.newInternalErrorBuilder().message(message, formatArgs).build())
        .build()
    }
    ...
    fun <V> BadRequestErr(message: String, vararg formatArgs: Any): DataFetcherResult<V> {
      return DataFetcherResult.newResult<V>()
        .error(TypedGraphQLError.newBadRequestBuilder().message(message, formatArgs).build())
        .build()
    }
