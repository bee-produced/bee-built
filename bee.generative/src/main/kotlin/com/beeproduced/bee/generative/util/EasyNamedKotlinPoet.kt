...
class PoetMap {
  ...
  val args
    get(): Map<String, Any> = poetMappings

  val mappings
    get(): Map<String, Any> = originalMappings
  ...
  fun addMappings(map: Map<String, Any>) {
    ...
    map.mapKeys { (k) -> mappingKey(k) }.let { m -> poetMappings.putAll(m) }
  }
  ...
  private fun mappingKey(input: String): String {
    ...
    return if (withoutPrefix.contains(":"))
      withoutPrefix.substring(0, withoutPrefix.lastIndexOf(":"))
    else withoutPrefix
  }
