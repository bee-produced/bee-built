...
class CircularSelectionATest(
  ...
  @Autowired val semiCircular1Repo: SemiCircular1Repository,
  @Autowired val semiCircular2Repo: SemiCircular2Repository,
) {
  ...
  fun `test semi circular`() {
    transaction.executeWithoutResult {
      ...
      semiCircular1Repo.cbf
        .update(em, SemiCircular1::class.java)
        .set("cId", id2)
        .where("id")
        .eq(id1)
        .executeUpdate()
      ...
      val selection =
        SemiCircular1DSL.select { this.circular { this.circular { this.circular {} } } }
  ...
  fun `test semi circular 2`() {
    transaction.executeWithoutResult {
      ...
      val selection =
        SemiCircular1DSL.select { this.circular { this.circular { this.circular {} } } }
      val circle = semiCircular1Repo.select(selection).firstOrNull { it.id == id3 }
      assertNotNull(circle)
  ...
  fun `test semi circular 3`() {
    transaction.executeWithoutResult {
      ...
      semiCircular1Repo.cbf
        ...
        .eq(id1)
        .executeUpdate()
      semiCircular2Repo.cbf
        .update(em, SemiCircular2::class.java)
        .set("cId", id3)
        .where("id")
        .eq(id2)
        .executeUpdate()
      ...
      val selection =
        SemiCircular1DSL.select { this.circular { this.circular { this.circular {} } } }
      val circle = semiCircular1Repo.select(selection).firstOrNull { it.id == id1 }
      assertNotNull(circle)
