...
class OneToManyTest(
  ...
  @Autowired val workRepo: WorkRepository,
  @Autowired val collectionRepo: WorkCollectionRepository,
) {
  ...
  fun `select with relation loaded`() {
    ...
    transaction.executeWithoutResult {
      val selection =
        WorkCollectionDSL.select { this.works { this.workCollection { this.works {} } } }

      // TODO: Replace with selectById
      val collection =
        collectionRepo
          .select(selection) { where(WorkCollectionDSL.id.eq(collectionId)) }
          .firstOrNull()
  ...
  fun `select with joined where clause`() {
    ...
    transaction.executeWithoutResult {
      ...
      val collection =
        collectionRepo
          .select(selection) { where(WorkCollectionDSL.works.id.eq(workId2)) }
          .firstOrNull()
  ...
  fun `select by ids with relation loaded`() {
    ...
    transaction.executeWithoutResult {
      ...
      val collections =
        collectionRepo.select(selection) {
          whereOr(WorkCollectionDSL.id.eq(collectionId1), WorkCollectionDSL.id.eq(collectionId2))
        }
  ...
  fun `persist all and select with relation loaded`() {
    ...
    transaction.executeWithoutResult {
      ...
      val selection =
        WorkDSL.select { this.workCollection { this.works { this.workCollection {} } } }

      val pstWorks =
        listOf(work1, work2).map {
          workRepo.select(selection) { where(WorkDSL.id.eq(it.id)) }.first()
        }
  ...
  fun `update field`() {
    ...
    transaction.executeWithoutResult {
      val work = workRepo.select { where(WorkDSL.id.eq(workId1)) }.firstOrNull()
      assertNotNull(work)
    ...
    transaction.executeWithoutResult {
      val workUpdate = workRepo.select { where(WorkDSL.id.eq(workId1)) }.firstOrNull()
  ...
  private fun assertWorkCollection(
    ...
    // TODO: Investigate in the future: Not loaded relations are in some cases empty collections
    // instead of `null`
    assertNotNull(works)
  ...
  private fun assertWork(work: Work, collectionId: Long, workIds: Set<WorkId>, depth: Int) {
