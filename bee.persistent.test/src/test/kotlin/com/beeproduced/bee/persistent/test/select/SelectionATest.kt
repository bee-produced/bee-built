...
class SelectionATest(
    ...
    @Autowired
    val companyPersonRepo: CompanyPersonRepository,
    @Autowired
    ...
    private fun addSong() {
        transaction.executeWithoutResult {
            ...
            val company = companyRepo.persist(
                Company(UUID.randomUUID(), null)
            )
            val company2 = companyRepo.persist(
                Company(UUID.randomUUID(), null)
            )
    ...
    private fun assertSong(song: Song, selection: BeeSelection) {
        ...
        if (selection.contains(Song::interpret.name)) {
            ...
            if (subselect != null)
                assertPerson(interpret, subselect)
        } else {
        ...
        if (selection.contains(Song::producer.name)) {
            ...
            if (subselect != null)
                assertCompany(producer, subselect)
        } else {
            assertNull(producer)
        }
    }
        ...
        if (selection.contains(Person::companies.name)) {
            ...
            if (subselect != null)
                for (companyPerson in companyPersons)
                    assertCompanyPerson(companyPerson, subselect)
        } else {
    ...
    private fun assertCompanyPerson(companyPerson: CompanyPerson, selection: BeeSelection) {
        ...
        if (selection.contains(CompanyPerson::company.name)) {
            ...
            if (subselect != null)
                assertCompany(company, subselect)
        } else {
            assertNull(company)
        }
