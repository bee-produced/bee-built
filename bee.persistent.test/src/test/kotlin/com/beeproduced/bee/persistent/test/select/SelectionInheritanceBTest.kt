...
class SelectionInheritanceBTest(
    ...
    private fun addComposers() {
        transaction.executeWithoutResult {
            ...
            val aiComposer = composerRepo.persist(AiComposer(UUID.randomUUID(), "AI", "GPT", AiParams("1", "2"), aiData.id))
            val humanData = humanDataRepo.persist(HumanData(UUID.randomUUID(), "Bar"))
            val humanComposer = composerRepo.persist(HumanComposer(UUID.randomUUID(), "Mario", "Mario", humanData.id))
            val container = containerRepo.persist(ComposerContainer(UUID.randomUUID(), aiComposer.id, null, humanComposer.id, null))
        }
    }
