...
class SelectWhereATest(
  ...
  @Autowired val circularRepo: CircularRepository,
  ...
  fun `where`() {
    transaction.executeWithoutResult {
      ...
      val circles = circularRepo.select(selection) { where(CircularDSL.id.eq(id2)) }
  ...
  fun whereAnd() {
    transaction.executeWithoutResult {
      ...
      val selection = CircularDSL.select { this.circular { this.circular {} } }
      val circles =
        circularRepo.select(selection) { whereAnd(CircularDSL.id.eq(id1), CircularDSL.id.eq(id2)) }
  ...
  fun `whereOr with inner whereAnd`() {
    transaction.executeWithoutResult {
      ...
      val selection = CircularDSL.select { this.circular { this.circular {} } }
      val circles =
        circularRepo.select(selection) {
          whereOr(
            CircularDSL.id.eq(id1),
            and(CircularDSL.id.eq(id2), CircularDSL.id.eq(UUID.randomUUID())),
          )
        }
    ...
    transaction.executeWithoutResult {
      ...
      val circles =
        circularRepo.select(selection) {
          whereOr(
            CircularDSL.id.eq(id1),
            or(CircularDSL.id.eq(id2), CircularDSL.id.eq(UUID.randomUUID())),
          )
        }
  ...
  fun `where with inline value class`() {
    transaction.executeWithoutResult {
      ...
      val ws = weirdRepo.select { where(WeirdClassDSL.foxtrot.eq("Foxtrot")) }
      assertEquals(1, ws.count())
      val w = ws.first()
      assertEquals(id, w.id)

      val wsInline = weirdRepo.select { where(WeirdClassDSL.foxtrot.eqInline(Foxtrot("Foxtrot"))) }
  ...
  fun `lower expression`() {
    transaction.executeWithoutResult {
      ...
      val ws = weirdRepo.select { where(lower(WeirdClassDSL.foxtrot).eq("Foxtrot".lowercase())) }
      assertEquals(1, ws.count())
      val w = ws.first()
      assertEquals(id, w.id)
    }
