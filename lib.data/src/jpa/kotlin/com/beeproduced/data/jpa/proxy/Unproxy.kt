...
package com.beeproduced.data.jpa.proxy

import com.beeproduced.data.jpa.meta.MemberInfo
import com.beeproduced.data.jpa.meta.MetaModel
...
object Unproxy {
    ...
    private class VisitedKey(
        val type: Class<*>, val id: Any
    ) {
    ...
    private fun unproxyMember(
        ...
        for (target in targets) {
            ...
            if (isInitialized(possibleProxyEntity)) {
                ...
                val entity = if (possibleProxyEntity is HibernateProxy) {
    ...
    private fun isInitialized(entity: Any?): Boolean {
        ...
        // Reasoning: Query described in https://git.beeproduced.com/beeproduced/backend-spring/-/issues/114
        // loads unexpectedly the company of CompanyMember even when it is not selected.
        // It is also not "fully loaded", normally with the loadgraph API no proxy but the object is returned
        // but in this case an unexpected proxy is returned instead of null.
        // Even though it is a proxy but more own lazy loaded properties it is marked internally as initialized.
        // Update: 15.06.2023
        // Query described here https://git.beeproduced.com/beeproduced/backend-spring/-/issues/176
        // returns proxy even if selected in loadgraph, therefore reverting to old behavior but additionally
